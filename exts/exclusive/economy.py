...
class Economy(commands.Cog, command_attrs=dict(hidden=True)):
    ...
    async def cog_check(self, ctx) -> bool:
        ...
        return (
            ctx.channel.name in ["bot-test", "botspam-v2", "botspam"]
    ...
    def get_item_column(self, inp: str) -> str:
        ...
        mapping = {"stock": "stock", "heist tools": "heist", "chicken": "chicken"}
    ...
    async def chat_money_loop(self):
        ...
        await self.bot.db_pool.executemany(
            ...
            [
                (v * random.randint(1, 15), k)
                for k, v in self.bot.chat_money_cache.items()
            ],
        )
    ...
    async def stock_price(self):
        message_channel = self.bot.get_channel(
            int(self.bot.environ.get("BOT_ANNOUNCE_CHANNEL"))
        )
        ...
        new = await self.bot.db_pool.fetchval(
            f"UPDATE shop SET price=price + $1 WHERE item_name='stock' RETURNING price",
            rand_val,
        )
    ...
    @commands.command(
        name="withdraw", aliases=["with"], help="Withdraw money from your account."
    )
        ...
        try:
            ...
            if existing["bank"] >= amount:
                await self.bot.db_pool.execute(
                    "UPDATE economy SET cash=cash + $1, bank=bank - $1 WHERE user_id=$2",
                    amount,
                    ctx.author.id,
                )
                response = EconomyEmbed(
                    title="Withdrawal",
                    description=f"Withdrew {int(amount)}",
                ...
                raise ValueError(
                    f"You do not have that much balance; you're {amount - existing['bank']} short."
                )
        except TypeError:
            if amount.lower() == "all":
                await self.bot.db_pool.execute(
                    "UPDATE economy SET cash=cash + bank, bank=0 WHERE user_id=$1",
                    ctx.author.id,
                )
                response = EconomyEmbed(
                    title="Withdrawal",
                    description=f"Withdrew all money from bank.",
                    embed_type=enums.EmbedType.BOT,
                )
                response.quick_set_author(ctx.author)
                await ctx.reply(embed=response)
    ...
    @commands.command(
        name="deposit", aliases=["dep"], help="Deposit money to your account."
    )
    async def deposit(self, ctx, amount: typing.Union[int, str]):
        ...
        try:
            if existing["cash"] >= amount:
                await self.bot.db_pool.execute(
                    "UPDATE economy SET bank=bank + $1, cash=cash - $1 WHERE user_id=$2",
                    amount,
                    ctx.author.id,
                )
                response = EconomyEmbed(
                    title="Deposit",
                    description=f"Deposited {int(amount)} to bank.",
                    embed_type=enums.EmbedType.BOT,
                )
                response.quick_set_author(ctx.author)
                await ctx.reply(embed=response)
            ...
            if amount.lower() == "all":
                await self.bot.db_pool.execute(
                    "UPDATE economy SET bank=cash + bank, cash=0 WHERE user_id=$1",
                    ctx.author.id,
                )
                response = EconomyEmbed(
                    title="Deposit",
                    description=f"Deposited {existing['cash']} to bank.",
                    embed_type=enums.EmbedType.BOT,
                )
                response.quick_set_author(ctx.author)
                await ctx.reply(embed=response)
    ...
    @commands.command(
        name="balance", aliases=["bal"], help="Displays your current bank balance."
    )
    async def balance(self, ctx, user: typing.Union[discord.Member, str] = None):
        if user is None:
            user = ctx.author
        user_data = await self.bot.db_pool.fetchrow(
            "SELECT cash, bank, cash + bank - debt AS networth, debt FROM economy WHERE user_id=$1",
            user.id,
        )

        response = EconomyEmbed(
            title="Crajy Bank",
            description="Balance is:",
            embed_type=enums.EmbedType.BOT,
        )
        ...
        response.add_field(
            name="Net Worth: ", value=user_data["networth"], inline=False
        )
        response.quick_set_author(user)
        return await ctx.reply(embed=response)

    @commands.command(name="work", help="Do work to earn some money.")
    ...
    async def work(self, ctx):
        ...
        response = EconomyEmbed(
            title="Work",
            description=f"You earned {rand_val}",
            embed_type=enums.EmbedType.SUCCESS,
        )
        response.quick_set_author(ctx.author)
        return await ctx.reply(embed=response)

    @commands.command(name="slut", help="Be a slut to earn that dough")
    @commands.cooldown(1, 3600, commands.BucketType.user)
    async def slut(self, ctx):
        ...
        if random.randint(1, 10) in winning_odds:
            ...
            response = EconomyEmbed(
                title="You slut.",
                description=f"You whored out and earned {rand_val}!",
                embed_type=enums.EmbedType.SUCCESS,
            )
        else:
            ...
            response = EconomyEmbed(
                title="Uh oh...",
                description=f"You hooked up with a psychopath, lost {rand_val}!",
    ...
    @commands.command(name="crime", help="Commit a crime to earn money.")
    @commands.cooldown(1, 3600, commands.BucketType.user)
    async def crime(self, ctx):
        ...
        if random.randint(1, 10) in winning_odds:
            ...
            response = EconomyEmbed(
                title="Gang shit bro.",
                description=f"You successfuly commited crime and earned {rand_val}!",
                embed_type=enums.EmbedType.SUCCESS,
            )
            response.quick_set_author(ctx.author)
            ...
            response = EconomyEmbed(
                title="You're kinda stupid bro.",
                description=f"You got caught and were fined {rand_val}!",
                embed_type=enums.EmbedType.FAIL,
            )
            response.set_thumbnail(url=em.EmbedResource.LOSS.value)
        response.quick_set_author(ctx.author)
    ...
    @commands.command(
        name="leaderboard", aliases=["top", "lb"], help="Economy leaderboard."
    )
    async def leaderboard(self, ctx):
        leaderboard_data = await self.bot.db_pool.fetch(
            "SELECT user_id, bank + cash - debt AS networth FROM economy ORDER BY networth DESC"
        )
        embeds = []
        counter = 1
        for chunk in mitertools.chunked(leaderboard_data, 6):
            response = EconomyEmbed(
                title="Crajy Leaderboard",
                description="",
            ...
            for person in chunk:
                ...
                if person_obj is None:
                    continue
                response.add_field(
                    name=f"{counter}. {person_obj.display_name}",
                    value=f"Net Worth: {person['networth']}",
                    inline=False,
                )
    ...
    @commands.command(name="repay-loan", aliases=["rl"], help="Repay your loan.")
    ...
    @commands.command(
        name="inventory", aliases=["inv"], help="View your inventory of items."
    )
    async def inventory(self, ctx, user: discord.Member = None):
        if user is None:
            user = ctx.author
        user_data = await self.bot.db_pool.fetchrow(
            "SELECT * FROM inventories WHERE user_id = $1", user.id
        )
    ...
    @commands.command(name="shop", aliases=["store"], help="View the store.")
    async def shop(self, ctx):
        shop_data = await self.bot.db_pool.fetch(
            "SELECT item_name, stock, price FROM shop"
        )
        embeds = []
    ...
    @commands.command(
        name="buy", help="Buy an item from the shop."
    )  # IMPORTANT!! - For items that should have unlimited stock, use stock value as None in store_data collection.
    async def buy(self, ctx, number: int, *, item: str):
        if not self.not_negative(number):
            raise ValueError("Negative numbers are not allowed.")
        store_data = await self.bot.db_pool.fetchrow(
            "SELECT stock, price FROM shop WHERE item_name = $1", item.lower()
        )
        user_cash_data = await self.bot.db_pool.fetchrow(
        ...
        if user_cash_data["cash"] >= (number * store_data["price"]):
            if store_data["stock"] is not None:
                if store_data["stock"] >= number:
                    ...
                    await self.bot.db_pool.execute(
                        f"UPDATE inventories SET {self.get_item_column(item)} = {self.get_item_column(item)} + $1 WHERE user_id = $2",
                        number,
                        ctx.author.id,
                    )
                    await self.bot.db_pool.execute(
                        "UPDATE shop SET stock = stock - $1 WHERE item_name = $2",
                        number,
                        item,
                    )
                    response = EconomyEmbed(
                        title="Purchase Successful",
                        description=f"You bought {number} {item}s!",
                        embed_type=enums.EmbedType.SUCCESS,
                    )
                    response.quick_set_author(ctx.author)
                    return await ctx.reply(embed=response)
            ...
            raise ValueError(
                f"poopi you don't have enough moni {self.bot.get_emoji(703648812669075456)}"
            )

    @commands.command(name="sell", help="Sell an item for the current market price.")
    async def sell(self, ctx, n: int, item: str):
        if not self.not_negative(n):
            raise ValueError("Negative numbers are not allowed.")
        cur_price = await self.bot.db_pool.fetchval(
            "SELECT price FROM shop WHERE item_name = $1", item
        )
        await self.bot.db_pool.execute(
            f"UPDATE inventories SET {self.get_item_column(item)} = {self.get_item_column(item)} - $1 WHERE user_id = $2",
            n,
        ...
        response = EconomyEmbed(
            title="Item Sold.",
            description=f"You sold {n} {item}s for {cur_price * n}",
            embed_type=enums.EmbedType.SUCCESS,
        )
        response.quick_set_author(ctx.author)
    ...
    async def givemoney(self, ctx, person: discord.Member, amount: int):
        if not self.not_negative(amount):
            raise ValueError("Negative numbers are not allowed.")
        sender_balance = await self.bot.db_pool.fetchval(
            "SELECT cash FROM economy WHERE user_id = $1", ctx.author.id
        )
        if sender_balance < amount:
            raise ValueError(
                f"You don't have that much money; You're short by {amount - sender_balance}"
            )
        ...
        if amount < 0:
            ...
            response = EconomyEmbed(
                ...
                description=f"{ctx.author.mention} transferred {int(amount)} to {person.mention}",
                embed_type=enums.EmbedType.BOT,
            )
            response.set_thumbnail(url=em.EmbedResource.PAYMENT.value)
        return await ctx.maybe_reply(embed=response, mention_author=True)

    @commands.command(name="rob", aliases=["steal"], help="Rob your friends.")
    @commands.cooldown(1, 3600, commands.BucketType.user)
    async def rob(self, ctx, person: discord.Member):
        robber_heist_tools = await self.bot.db_pool.fetchval(
            "SELECT heist FROM inventories WHERE user_id = $1", ctx.author.id
        )
        ...
        if robber_heist_tools > 0 and victim["cash"] > 10:
            await self.bot.db_pool.fetchval(
                "UPDATE inventories SET heist = heist - 1 WHERE user_id = $1",
                ctx.author.id,
            )
            ...
            if won:
                ...
                response = EconomyEmbed(
                    title="Heist!",
                    description=f"You robbed {win_amount} from {person.mention}",
                    embed_type=enums.EmbedType.SUCCESS,
                )
                response.quick_set_author(ctx.author)
                ...
                response = EconomyEmbed(
                    title="Uh oh...",
                    description=f"You were caught robbing, and fined {fine_amount}.",
                    embed_type=enums.EmbedType.FAIL,
                )
                response.quick_set_author(ctx.author)
