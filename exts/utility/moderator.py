...
class Moderator(commands.Cog):
    ...
    @commands.group(
        name="change-money", aliases=["c-money", "cm"], help="Change a user's balance."
    )
    ...
    async def add_money(self, ctx, baltype: str, user: discord.Member, amt: int):
        await self.bot.db_pool.execute(
            f"UPDATE economy SET {baltype}={baltype} + $1 WHERE user_id=$2",
            amt,
            user.id,
        )
        response = em.CrajyEmbed(
            title="Updating User Balance",
            description=f"Added {amt} to {user.display_name}'s {baltype}.",
    ...
    async def remove_money(self, ctx, baltype: str, user: discord.Member, amt: int):
        ...
        response = em.CrajyEmbed(
            title="Updating User Balance",
            description=f"Removed {amt} from {user.display_name}'s {baltype}.",
    ...
    async def set_money(self, ctx, baltype: str, user: discord.Member, amt: int):
        await self.bot.db_pool.execute(
            f"UPDATE economy SET {baltype}=$1 WHERE user_id=$2", amt, user.id
        )
        response = em.CrajyEmbed(
            title="Updating User Balance",
            description=f"Set {user.display_name}'s {baltype} to {amt}.",
    ...
    @commands.group(
        name="change-inventory", aliases=["c-inv"], help="Edit a user's inventory."
    )
    ...
    async def add_inv(self, ctx, amt: int, item: str, user: discord.Member):
        await self.bot.db_pool.execute(
            f"UPDATE inventories SET {item}={item} + $1 WHERE user_id=$2", amt, user.id
        )
        response = em.CrajyEmbed(
            title="Updating User Inventory",
            description=f"Added {amt} {item} to {user.display_name}'s inventory.",
            embed_type=enums.EmbedType.SUCCESS,
        )
        response.quick_set_author(ctx.author)
        response.set_thumbnail(url=em.EmbedResource.GREEN_UPDATE.value)
    ...
    async def remove_inv(self, ctx, amt: int, item: str, user: discord.Member):
        ...
        response = em.CrajyEmbed(
            title="Updating User Inventory",
            description=f"Remove {amt} {item} from {user.display_name}'s inventory.",
            embed_type=enums.EmbedType.FAIL,
        )
        response.quick_set_author(ctx.author)
        response.set_thumbnail(url=em.EmbedResource.RED_UPDATE.value)
    ...
    async def set_inv(self, ctx, item: str, user: discord.Member, amt: int):
        await self.bot.db_pool.execute(
            f"UPDATE inventories SET {item}=$1 WHERE user_id=$2", amt, user.id
        )
        response = em.CrajyEmbed(
            title="Updating User Inventory",
            description=f"Set {amt} {item} to {user.display_name}'s inventory.",
            embed_type=enums.EmbedType.BOT,
        )
        response.quick_set_author(ctx.author)
        response.set_thumbnail(url=em.EmbedResource.GREEN_UPDATE.value)
    ...
    @commands.command(
        name="edit-item", aliases=["edititem"], help="Edit an item on the shop."
    )
    async def edit_item(self, ctx, item: str, attribute: str, value: int):
        await self.bot.db_pool.execute(
            f"UPDATE shop SET {attribute}=$1 WHERE item_name=$2", value, item
        )
        response = em.CrajyEmbed(
            title="Updating Shop",
            description=f"Set {item} {attribute} to {value}.",
            embed_type=enums.EmbedType.BOT,
        )
        response.quick_set_author(ctx.author)
        response.set_thumbnail(url=em.EmbedResource.GREEN_UPDATE.value)
    ...
    async def remove_pins(
        self, ctx, identifiers: commands.Greedy[typing.Union[int, str]]
    ):
        ids, names = [], []
        ...
        confirm_embed = em.CrajyEmbed(
            title="Clearing Pins", embed_type=enums.EmbedType.WARNING
        )
        ...
        if not response:
            ...
            if len(identifiers) == 1 and identifiers[0].lower() == "all":
                ...
                await self.bot.db_pool.execute(
                    "DELETE FROM pins WHERE name=ANY($1) OR pin_id=ANY($2)", names, ids
                )

    @commands.command(
        name="pin",
        help="Pins a message to the bot's database. Pins can be viewed with the `pins` command.",
    )
    async def pin(self, ctx, id_: discord.Message, name_: str = None):
        ...
        await self.bot.db_pool.fetchval(
            "INSERT INTO pins(synopsis, jump_url, author, pin_date, name) VALUES($1, $2, $3, $4)",
            synopsis,
            url,
            author,
            date,
        ...
        reply_embed = em.CrajyEmbed(
            title=f"Pinned!",
            description=f"_{synopsis[:10]+'...'}_\n",
            embed_type=enums.EmbedType.SUCCESS,
        )
    ...
    async def clear(
        ...
        ctx,
        amount: int,
        filter: commands.Greedy[converters.KwargConverter] = None,
    ):
        if filter:

            def check(message):
                merged_kwargs = {
                    key: value for inner in filter for key, value in inner.items()
                }
                results = []
        ...
        return await ctx.send(
            f"{em.EmbedResource.CHECK_EMOJI.value} Cleared {len(deleted)} messages.",
            delete_after=5,
        )
