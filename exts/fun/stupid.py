...
class Stupid(commands.Cog):
    ...
    async def fancy(self, ctx, *, message):
        ...
        async with ctx.channel.typing():
            async with self.bot.session.get(
                fancy_url, headers=fancy_headers, params=querystring
            ) as response:
    ...
    async def love_calc(self, ctx, sname: str, fname=None):
        ...
        async with ctx.channel.typing():
            async with self.bot.session.get(
                love_url, headers=love_headers, params=querystring
            ) as response:
    ...
    @commands.command(
        name="pins",
        help="Display the messages pinned in the bot database. Useful if your channel has already reached the 50 pin limit.",
    )
    async def pins(self, ctx):
        data = await self.bot.db_pool.fetch(
            "SELECT pin_id, synopsis, jump_url, author, pin_date FROM pins"
        )
        ...
        for chunk in mitertools.chunked(data, 6):
            embed = em.CrajyEmbed(
                title=f"{ctx.guild.name} Pins!", embed_type=enums.EmbedType.INFO
            )
    ...
    @commands.command(
        name="fetch-pin",
        aliases=["fetchpin"],
        help="Return a pin based on the ID provided. WIP.",
    )
    async def fetch_pin(self, ctx, identifier: Union[str, int]):

        if identifier.isalpha():
            data = await self.bot.db_pool.execute(
                "SELECT pin_id, synopsis, jump_url, pin_date FROM pins WHERE name=$1",
                identifier,
            )
        elif identifier.isdigit():
            data = await self.bot.db_pool.execute(
                "SELECT pin_id, synopsis, jump_url, pin_date FROM pins WHERE pin_id=$1",
                identifier,
            )

        embed = em.CrajyEmbed(
            title=f"Pin **{data['_id']}**",
            url=data["jump_url"],
    ...
    @commands.command(
        name="change-vote-threshold",
        help="Change the minimum votes required to pin a message.",
    )
    ...
    @commands.group(
        name="role-name", aliases=["rolename", "rolenames"], invoke_without_command=True
    )
    async def role_name(self, ctx, *, name: str):

        await self.bot.db_pool.execute(
            "INSERT INTO role_names(role_name, author) VALUES ($1, $2)",
            name,
            ctx.author.id,
        )
        ...
        embed = em.CrajyEmbed(
            title="Added!",
            description=f"`{name}` was added to the database. It will be picked randomly.",
            embed_type=enums.EmbedType.SUCCESS,
            url=r"https://www.youtube.com/watch?v=DLzxrzFCyOs",
        )
        embed.quick_set_author(ctx.author)
    ...
    @commands.has_guild_permissions(administrator=True)
    async def role_name_remove(
        self, ctx, *names: str
    ):  # can pass multiple role names to bulk delete. each arg must be wrapped in quotes.
        await self.bot.db_pool.execute(
            "DELETE FROM role_names WHERE LOWER(role_name) = ANY($1)", names
        )

        embed = em.CrajyEmbed(
            title="Deleting Role Names", embed_type=enums.EmbedType.BOT
        )
        ...
        embed.set_footer(
            text="Note: Names are deleted if they existed in the database in the first place."
        )
