...
class Games(commands.Cog):
    ...
    async def ttt(self, ctx, opponent: discord.Member = None):
        ...
        players = {
            tuple(tictactoe.X): ctx.author,
            tuple(tictactoe.O): opponent,
        }  # check cause of error because tuple() is unnecesary

        main_message_embed = discord.Embed(
            title="TicTacToe Game!",
            description=f"{ctx.author.mention} has challenged {opponent.mention}!\n {players[tuple(player)]} makes the first move.",
            timestamp=datetime.datetime.utcnow(),
        )
        main_message_embed.set_thumbnail(
        ...
        while not tictactoe.terminal(board):
            reaction, _ = await self.bot.wait_for(
                "reaction_add", check=player_check, timeout=180
            )
            ...
            if reaction:
                ...
                main_message_embed = tictactoe.update_board(
                    main_message_embed, reaction.emoji, player
                )
                player, next_player = next_player, player
                main_message_embed.description += (
                    f"\n{players[player]}'s turn"  # maybe move to better place
                )
                await main_message.edit(embed=main_message_embed)

        main_message_embed.description = (
            f"{players[next_player]} destroyed {players[player]}!\n Good Game!"
        )
        await self.bot.games_leaderboard.update_one(
            {"user": players[next_player].id}, {"$inc": {"wins": 1}}, upsert=True
        )
    ...
    async def guess(self, ctx):
        ...
        def answer_check(m):
            ...
            if (
                ...
                and m.content.lower() == answer_val.lower()
            ):
                return True

        await ctx.author.send(
            "Send the word that everyone has to guess! Send 'plshelp' if you don't have a word."
        )
        try:
            answer = await self.bot.wait_for("message", check=reply_check, timeout=30)
            if answer.content == "plshelp":
                await ctx.author.send(
                    "Note: the auto word picking thing may not always work (no word might be sent to the channel, incorrect meaning) etc so, dont blame me too much :("
                )
                try:
                    async with ctx.author.dm_channel.typing():
                        answer_val = r.get_random_word(
                            hasDictionaryDef="true", maxLength=6
                        )
                        clue_dict = d.meaning(answer_val)
                        answer_val = answer_val.lower()
                    clue_val = list(clue_dict.values())
                    await ctx.author.dm_channel.send(
                        f"Chosen a word! The word is **{answer_val}**"
                    )
                except:
                    await ctx.author.send(
                        "whoops something fucked up. Starting the command again 😔...."
                    )
                    return await self.guess(ctx)
                await ctx.send(
                    f"{ctx.author.mention} has chosen a word! Everyone has 1 minute to guess it."
                )
                await ctx.send(f"Clue - **{clue_val}**")
                try:
                    reply = await self.bot.wait_for(
                        "message", check=answer_check, timeout=60
                    )
                except asyncio.TimeoutError:
                    return await ctx.send(
                        f"Time up! No one guessed the word. The word was **{answer_val}**"
                    )

                await self.bot.games_leaderboard.update_one(
                    {"user": reply.author.id}, {"$inc": {"wins": 1}}, upsert=True
                )
                return await ctx.send(
                    f"{reply.author.mention} got it right! The word was **{reply.content}**"
                )
    ...
    async def akinator_game(self, ctx):
        ...
        game_embed = discord.Embed(
            title=f"{str(ctx.author.nick)}'s game of Akinator",
            description=q,
        ...
        option, _ = await self.bot.wait_for(
            "reaction_add", check=option_check, timeout=10
        )
        if option.emoji == "✅":
            final_embed = discord.Embed(
                title="I'm a fuckin genius", color=discord.Color.green()
            )
        elif option.emoji == "❌":
            final_embed = discord.Embed(
