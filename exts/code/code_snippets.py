"""Send code snippets in chat. This code is taken from https://github.com/python-discord/bot."""
import logging
import re
import textwrap
...
class CodeSnippets(Cog):
    ...
    def _snippet_to_codeblock(
        ...
        if end_line is None:
            start_line = end_line = int(start_line)  # type: ignore
        else:
            start_line = int(start_line)  # type: ignore
            end_line = int(end_line)  # type: ignore
        ...
        if start_line > len(split_file_contents) or end_line < 1:  # type: ignore
            return ""
        start_line = max(1, start_line)  # type: ignore
        end_line = min(len(split_file_contents), end_line)  # type: ignore

        # Gets the code lines, dedents them, and inserts zero-width spaces to prevent Markdown injection
        required = "\n".join(split_file_contents[start_line - 1 : end_line])  # type: ignore
