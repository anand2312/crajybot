...
class Minecraft(commands.Cog):
    def __init__(self, bot):
        ...
        self.embed = em.CrajyEmbed(title="Minecraft Server", embed_type=enums.EmbedType.INFO)
        self.embed.set_thumbnail(url=em.EmbedResource.MINECRAFT.value)
    ...
    async def start_server(self, ctx, override: str=False):
        if override != "--override":
            if self.running:
                ...
                raise Exception("Server already running! IP: 35.241.144.6")
            cur_time = time.time()
            diff = (cur_time - self.init_time) // 60
            if diff < 6:          # applies a 6 minute cooldown on startup command after a shutdown
                ctx.command.reset_cooldown(ctx)
                raise Exception(f"You're trying to start the server back up too quick. Time since last shutdown sequence ~{diff} mins")
        else:
        ...
        async with self.bot.session.get(START_LINK) as response:
            if response.status == 200:
                ...
                self.embed.color = enums.EmbedType.SUCCESS.value
    ...
    async def stop_server(self, ctx, override: str=False):
        if override != "--override":
            if not self.running:
                ctx.command.reset_cooldown(ctx)
                raise Exception("Server is not running right now.")
            cur_time = time.time()
            diff = (cur_time - self.init_time) // 60
            if diff < 6:     # applies a 6 minute cooldown on shutdown command since start-up
                ctx.command.reset_cooldown(ctx)
                raise Exception(f"You're trying to shut the server down too quick. Time since start-up sequence ~{diff} mins")
        else:
            if not ctx.author.guild_permissions.administrator:
                ctx.command.reset_cooldown(ctx)
                raise Exception("no.")
        ...
        async with self.bot.session.get(STOP_LINK) as response:
            if response.status == 200:
                ...
                self.embed.color = enums.EmbedType.FAIL.value
                self.embed.timestamp = datetime.datetime.now(tz=BOT_TZ)
                await ctx.send(embed=self.embed)
            self.running = False
            self.init_time = time.time()
    ...
    async def uptime(self, ctx):
        ...
        return await ctx.maybe_reply(f"Uptime: {diff} minutes") 
    ...
    async def server_variable_override(self, ctx, var, value):
        ...
        return await ctx.reply("Variables updated.", mention_author=True)
