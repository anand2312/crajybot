...
class Notes(commands.Cog):
    ...
    async def notes_create(self, ctx, *, content):
        current = await self.bot.notes_collection.find_one({"user": ctx.author.id})
        if current is None: current = "\n"
        else: current = current["notes"]
        await self.bot.notes_collection.update_one({"user":ctx.author.id},{"$set":{"notes":current + content + "\n"}}, upsert=True)
    ...
    async def notes_return(self, ctx):
        try:
            existing = await self.bot.notes_collection.find_one({"user":ctx.author.id})
            await ctx.author.send(existing["notes"])
    ...
    async def notes_pop(self, ctx):
        await self.bot.notes_collection.delete_one({"user":ctx.author.id})
